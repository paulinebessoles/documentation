...
feedback from participants. It allows defining a set of questions with multiple kinds of questions.

NOTE: Once a survey is already answered by participants, it's not possible to modify its questions, as that would affect the integrity
...
|Answer option
|Required
|This field propose the answer options to the questions that need to be chose by participants in order to realize the condition. 
